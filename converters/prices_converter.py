...
def convert_prices_result_to_df(get_prices_result: list[dict]) -> pd.DataFrame:
    ...
    assigned_df = df.assign(price=df['sizes'].apply(lambda x: x[0]['price']),
                            price_seller=df['sizes'].apply(lambda x: x[0]['discountedPrice']),
                            price_wb_club=df['sizes'].apply(lambda x: x[0]['clubDiscountedPrice'])
                            )
